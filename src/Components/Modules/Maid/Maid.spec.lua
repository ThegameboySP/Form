...
return function()
	describe("Adding tasks", function()
		...
		it("no id argument: should add task with internal id", function()
			local maid = Maid.new()
			expect( type(maid:GiveTask(NOOP)) ).to.equal("table")
		end)
		...
		it("GiveTasks: should add tasks in array then return self and ids", function()
			...
			local key = next(ids)
			expect(maid[key]).to.equal(NOOP)
			expect(ret).to.equal(maid)
			expect(ids[key]).to.equal(NOOP)
		end)
